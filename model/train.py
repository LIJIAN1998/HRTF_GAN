...
def train(config, train_prefetcher):
    ...
    netG = AutoEncoder(nbins=nbins, in_order=in_order, latent_dim=latent_dim, base_channels=128, num_features=512, out_oder=max_order).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        while batch_data is not None:
            ...
            if batch_index % int(critic_iters) == 0:
                ...
                #     with open("log.txt", "a") as f:
                #         f.write("domain db, change recons, hrtf!!!!!!!!!!!!!!!!!!!\n")
                #         f.write(f"recons: {recons}\n\n")
